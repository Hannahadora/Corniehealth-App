  ...
  <div class="overflow-y-auto bg-white">
    ...
    >
      ...
      <div class="flex flex-col p-3">
        <p class="text-sm mt-2">Select preferred provider</p>
        <icon-input
          autocomplete="off"
        ...
        <div
          ...
          v-for="(item, index) in columnsProxy"
          :key="index"
        >
          <span class="items-center w-full flex justify-between">
            <label class="flex items-center justify-between py-3 px-3">
              ...
              />
              <span class="block">
                <span class="text-xs font-bold float-left pl-3"
                  >{{ item.firstName }} {{ item.lastName }}
                  <br />
                  <span class="text-xs text-gray-300 font-bold"
                    >{{ item.jobDesignation }},{{ item.department }}</span
                  >
                </span>
              </span>
            </label>
            <div class="flex mr-4 -ml-32">
              <p
                class="cursor-pointer mr-2 text-xs text-danger"
                @click="showAvailable(item.id)"
              >
                View Availability
              </p>
              <p
    ...
    <availability v-model:visible="availableFilter" practitionerId:
    practitionerId />

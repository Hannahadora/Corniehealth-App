<template>
  <div class="w-full" v-bind="$attrs">
    <div class="container">
      ...
      <div class="w-full">
        <span
          class="flex border justify-between dd-item px-4 py-2 cursor-pointer"
          @click="showItems"
        >
          <span class="dd-item" :class="{ 'text-gray-500': !myValue }">{{
            !myValue
              ? placeholder
              : optionLabel
              ? myValue[optionLabel]
      ...
      <div class="w-full dd-item relative" v-if="drop">
        <div
          class="absolute w-full pb-2 bg-white dd-item rounded-md shadow-xl z-20"
        ...
        >
          <a
            ...
            :key="index"
            class="block cursor-pointer dd-item px-4 py-2 text-sm capitalize text-gray-700 hover:bg-blue-500 hover:text-white"
          >

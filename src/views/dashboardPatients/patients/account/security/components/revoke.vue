    ...
    >
      <cornie-card-title class="w-full">
        <div class="w-full flex items-center justify-between">
          <h2 class="font-bold float-left text-lg text-primary">Revoke</h2>
      ...
      <p class="my-4">Confirm you want to revoke access for this practice.</p>
...
export default class RevokeAccess extends Vue {
  ...
  async submit() {
    try {
      ...
      const res = await cornieClient().get(
        `/api/v1/patient-portal/provider-permission/revoke/${this.id}`
      );
      this.loading = false;
      if (!res.status) {
        notify({

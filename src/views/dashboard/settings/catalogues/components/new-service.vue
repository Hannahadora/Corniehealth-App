    ...
    <div class="container-fluid p-4 bg-white overflow-y-scroll">
        <div class="w-full">
            ...
            <div class="w-full flex">
                <div class="w-4/12">
                    <cornie-select :label="'Category'" v-model="reqBody.category" :items="[ 'Category 1' ]" />
                </div>
                <div class="w-4/12">
                    <cornie-select :label="'Sub-Category'" v-model="reqBody.subcategory" :items="[ 'Sub-Category 1' ]" />
                </div>
                <div class="w-4/12">
                    <service-dropdown @add="() => addNew = true" v-model="reqBody.name" :label="'Service Name'" :items="[ 'Specialist Consultation', 'General Consultation', 'Biopsy' ]" />
                </div>
            </div>
            ...
            <div class="w-full flex">
                ...
                <div class="w-4/12">
                    <cornie-input :label="'Item Code'" v-model="reqBody.itemCode" placeholder="--DNS Generated" />
                </div>
                <div class="w-4/12">
                    <div class="w-11/12 -mt-5">
                        <combo-input :label="'Service UoM'"  >
                            <template #list>
                                <cornie-select v-model="reqBody.serviceUOM" :items="[ 'hours']" style="width: 100%;border-radius: 8px 0 0 8px" />
                            </template>
                            <template #input>
                                <input-with-desc >
            ...
            <div class="w-full flex">
                <div class="w-4/12">
                    <cornie-input  v-model.number="reqBody.cost" :label="'Service Cost (NGN)'" />
                </div>
                <div class="w-4/12">
                    <cornie-input :label="'Free Markup'" placeholder="--Autoloaded--" />
                </div>
                <div class="w-4/12">
                    <cornie-input :label="'Discount Limit'" placeholder="--Autoloaded--" />
                </div>
            </div>
            ...
            <div class="w-full">
                <div class="w-full flex">
                    <label class="inline-flex items-center">
                        <input v-model="reqBody.applyVat" type="checkbox" class="form-radio h-3 w-3" :value="'Apply VAT to this service item'" >
            ...
            <div class="w-full p-4 bg-white" style="min-width: 400px">
                <div class="w-full">
                    <p class="modal-header flex justify-between">
                        <span>Add New</span>
                        <span class="cursor-pointer" @click="() => addNew = false"><close-icon /></span>
                    </p>
                    <p class="my-3 modal-text">Some modal text if necessary</p>
                </div>

                <div class="w-full my-4">
                    <cornie-input :label="'Name'" v-model="reqBody.name" style="width: 100%" />
                </div>

                <div class="w-full my-4">
                    <cornie-input :label="'Code'" v-model="reqBody.itemCode" style="width: 100%" />
                </div>
...
export default class NwService extends Vue {
    ...
    async onSave() {  
        ...
        try {
            ...
            if (done) {
                window.notify({ msg: `Service ${ this.reqBody?.id ? 'updated' : 'saved'} successfully`, status: "success" });
                this.$router.go(-1)
            } else {
    ...
    async created() {
        if (this.$route.params.serviceId) {
            ...
            this.reqBody = this.services.find(service => service.id === this.$route.params.serviceId) as ICatalogueService;

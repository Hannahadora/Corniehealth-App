<template>
    <div class="container-fluid " :class="{ 'mb-5': showDetails }">
        <div class="w-full">
            <p class="flex justify-between items-center cursor-pointer" @click="toggleDetailsDisplay" :class="{ 'border-b-2': showDetails || underlined }">
                <span class="header py-2" :style="{ color: color }">{{ title }}</span>
                <span class="flex ">
                    <ChevronDown :class="{ 'icon-down': showDetails, 'icon-normal': !showDetails }" /> 
                </span>
            </p>
            <div class="w-full show-details"  :class="{ 'mt-5': showDetails }" :style="{ height: displayHeight + 'px', overflow: showDetails && overflow ? '' : 'hidden' }">

<template>
  <div class="w-full">
   <accordion-component
            ...
            :height="200"
          >
          <cornie-table :columns="rawHeaders" v-model="sortDirectors" :listmenu="true" :check="false">
            <template #actions="{ item }">
              <div class="flex items-center hover:bg-gray-100 p-3 cursor-pointer"  @click="showDirectorEdit(item.id)">
    ...
    <director-modal :id="id" :directorId="directorId" v-model="showDirector" @director-added="directorAdded" @director-data="directorData"/>
  </div>
</template>
<script lang="ts">
...
showDirectorEdit(value:string){
 ...
 async directorAdded(){
  ...
  async deleteItem(id: string) {
    ...
    else window.notify({ msg: "Director details not deleted", status: "error" });
  }

  ...
  <div class="container mx-auto px-2">
    <div class="w-full border-b-2 curved flex py-2">
      ...
      <div class="w-2/12 flex items-center justify-end">
        <span @click="toggleModalVissibility" class="cursor-pointer"
          ><Icon :type="2"
        /></span>
      </div>
    </div>
      ...
      <Modal>
        ...
        <template v-slot:body>
          <p class="text-base text-gray-500">
            This action transfers the account ownership and all associated
            privileges to the selected user. Are you sure you wish to proceed?
          </p>
        </template>
          ...
          <div class="px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
            <Button>
              <button
                ...
                type="button"
                class="w-full inline-flex justify-center rounded-full border-transparent font-bold shadow-sm px-6 py-3 bg-red-600 text-base font-medium text-white hover:bg-red-700 focus:outline-none focus:none sm:ml-3 sm:w-auto sm:text-sm"
              >
                Yes
              </button>
            </Button>
            <Button>
              <button
                @click="toggleModal"
                type="button"
                class="mt-3 w-full inline-flex justify-center px-4 py-2 text-base text-white-700 font-bold hover:bg-gray-50 focus:outline-none focus:none sm:mt-0 sm:ml-3 sm:w-auto"
              >
                No
              </button>
            </Button>
    ...
    <Overlay :show="showTransferComfirmModal">
      <Modal>
        <template v-slot:header>
          <h3
            class="text-lg flex justify-between leading-6 font-medium capitalize text-gray-900 mb-5 modal_titlee"
            id="modal-title"
          >
            <span>Transfer admin right</span>
            <span
              @click="() => (showTransferComfirmModal = false)"
              class="lowercase pb-1 cursor-pointer font-normal bg-primary text-white flex items-center justify-center"
              style="width: 20px; height: 20px; border-radius: 50%"
              >×</span
            >
          ...
          <div class="w-full">
            <div class="container">
              <custom-dropdown
                ...
                :items="allContacts"
                :optionLabel="'name'"
                :label="'Select User/Practicioner'"
              />
        ...
        <template v-slot:buttons>
          <div class="px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
            ...
            <Button @click="() => (showTransferComfirmModal = false)">
              <button
                type="button"
                class="mt-3 w-full inline-flex justify-center px-4 py-2 text-base text-white-700 font-semibold hover:bg-gray-50 focus:outline-none focus:none sm:mt-0 sm:ml-3 sm:w-auto"
              >
                Cancel
              </button>
            </Button>
    ...
    <SideModal :show="showModal" @close-modal="toggleModalVissibility">
      ...
      <template v-slot:cancel>
        <Button :loading="false">
          <button
            @click="toggleModalVissibility"
            class="close_btn_border mr-3 text-gray-500 focus:outline-none font-bold py-3 px-8 rounded-full"
          >
            Close
          </button>
        </Button>
    ...
    <div class="w-full flex mb-12">
      <div class="w-8/12">
        <p>
          Here are roles you have created. These roles can be assigned to users,
          defining their access level within CornieHealth.
        </p>
      </div>
...
.role_card:hover .card_image,
.role_card:hover .card_round_image {
  border-radius: 12px;
  width: 44px;
  height: 44px;
  transition: all 0.5s ease-in-out;

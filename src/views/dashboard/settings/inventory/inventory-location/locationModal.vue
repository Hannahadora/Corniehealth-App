          ...
          <v-form ref="form">
            <div class="border-b-2 pb-5 border-dashed border-gray-200">
              <div class="w-full grid grid-cols-2 gap-5 mt-5 pb-5">
                <cornie-input label="Location Name" placeholder="--Enter--" class="w-full" v-model="name" />
                <cornie-select class="required" :rules="required" :items="[
                  'Hospital', 'Labs', 'Pharmacy'
                ]" label="Location Type" placeholder="--Select--" v-model="type">
                </cornie-select>
                ...
                <practioner-select class="w-full mb-2" label="Location Manager" v-model="manager">
                </practioner-select>
                <cornie-input label="Address" placeholder="--Enter--" class="w-full" v-model="address" />
                <!-- <cornie-select class="required" :rules="required" :items="[
                'CarePlan',
                'MedicationRequest',
                'ServiceRequest',
                ...
                <auto-complete :rules="required" v-model="country" required label="Country" :items="countries"
                  class="w-full" />
                <auto-complete required :rules="required" :items="states" v-model="state" label="State"
                  class="w-full" />
                <!-- <cornie-select class="required" :rules="required" :items="[
                'CarePlan',
                'MedicationRequest',
                ...
                <cornie-input label="City" :rules="required" placeholder="--Enter--" class="w-full" v-model="city" />

                <cornie-input label="Postal/Zip Code *" placeholder="--Enter--" class="w-full" v-model="zip" />
                <!-- <cornie-select class="required" :rules="required" :items="[
                  'CarePlan',
                  'MedicationRequest',
                  'ServiceRequest',
                ...
                <div class="flex flex-col space-y-2">
                  <div class="flex capitalize items-center mb-1 text-black text-sm font-semibold">Inventory Category
                  </div>
                  <Multiselect label="Inventory Category" v-model="category" mode="tags" :hide-selected="true"
                    :options="allInventoryCategories" placeholder="--Select--" class="w-full">
              ...
              <span @click="showInventoryRequest = true" class=" text-red-600 cursor-pointer">
                + Add New Category
              </span>

              <div class="grid grid-cols-3 w-full">
                <div v-for="(c, i) in displaySelectedCategories" :key="i" class="col-span-1">

        ...
        <cornie-card class="flex-grow scrollable">
          <div class="flex flex-col">
            <div class="flex flex-col space-y-1">
              <div>2 selected</div>
              <div>
                <icon-input class="border border-gray-600 py-2 rounded-full focus:outline-none"
                  placeholder="Search Table" v-model="filterQ">
            ...
            <div class="flex flex-col">
              <div v-for="(l, i) in LocationCategories" :key="i" class="flex flex-row space-x-2">
                <div class="flex-none">
                  <cornie-checkbox @click="select(l.id)" :checked="isSelected(l.id)" />
                </div>
                <div class="flex-1">
                  <div class="flex flex-col">
...
export default class addCategoryModal extends Vue {
  ...
  mounted() {
    const { data }: { data: string[] } = this.selectedItem
    this.selectedItems = Array.from(data)
  }
}
</script>
                ...
                <div class="w-full" style="overflow-y:scroll; height: 650px">
                    ...
                    <div class="w-full my-2 flex items-center">
                        <div class="w-6/12 -mt-3">
                            <cornie-input v-model="reqBody.instantiatesUri" :label="'Instantiate Uri'" />
                        </div>
                        <div class="w-6/12">
                            ...
                            <div class="w-6/12">
                                <div class="w-11/12 relative">
                                    <date-time-picker :label="'Start date & Time'" :width="'w-full'">
                                        <template #date>
                                            <span>{{ new Date(reqBody.startDate ?? Date.now()).toLocaleDateString()}}</span>
                                        </template>
                                        <template #time>
                                            <span>{{ '00:00' }}</span>
                                        </template>
                                        <template #input>
                                            <div class="w-full">
                                                <div class="w-full">
                                                    <v-date-picker v-model="reqBody.startDate" name="eeee" style="z-index:9000;width:100%"></v-date-picker>
                                                </div>
                                                <div class="w-full">
                                                    <label
                            ...
                            <div class="w-6/12">
                                <div class="w-11/12">
                                    <date-time-picker :label="'End date & Time'" :width="'w-full'">
                                        <template #date>
                                            <span>{{ new Date(reqBody.endDate ?? Date.now()).toLocaleDateString()}}</span>
                                        </template>
                                        <template #time>
                                            <span>{{ '00:00' }}</span>
                                        </template>
                                        <template #input>
                                            <v-date-picker name="eeee" v-model="reqBody.endDate"  style="z-index:9000;width:100%"></v-date-picker>
                                            <label
                                                class="block uppercase my-1 text-xs font-bold"
                                                >
                                                Time
                    ...
                    <div class="w-full my-2 flex items-center">
                        <div class="w-full flex items-center my-3">
                            <div class="w-4/12">
                                <div class="w-11/12 relative">
                                    <date-time-picker :label="'Created'" :width="'w-full'">
                                        <template #date>
                                            <span>{{ new Date(reqBody.scheduleTiming.startDate ?? Date.now()).toLocaleDateString()}}</span>
                                        </template>
                                        <template #time>
                                            <span>{{ '--:--' }}</span>
                                        </template>
                            ...
                            <div class="w-4/12">
                                <select-reference :label="'Author'" @selected="actorSelected"
                            ...
                            <div class="w-4/12">
                                <select-reference :label="'Contributor'" @selected="contributorSelected" :options="[ 'Practitioner', 'Patient', 'Location', 'Device' ]" />
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="w-full pt-5 my-4" v-if="selectedAuthor?.id || selectedContributor?.id" style="border-top: 1px dashed #C2C7D6;">
                        <div class="w-full flex">
                            <div class="w-4/12 px-2 box" v-if="selectedAuthor?.id">
                                <selected-author :selectedAuthor="selectedAuthor" :title="'Author'" />
                            </div>
            ...
            <template #form>
                <div class="w-full">
                    <div class="w-full my-2 flex">
                        <div class="w-6/12">
                            <auto-complete v-model="reqBody.outcome" :label="'Outcome'" :items="[ 'Appointment', 'Communication Request', 'Device Request', 'MedicationRequest' ]" />
                        </div>
                        <div class="w-6/12">
                            <select-reference @selected="outcomeReferenceSelected" :label="'Outcome Reference'" :options="[ 'Appointment', 'Communication Request', 'Device Request', 'MedicationRequest' ]" :selectedType="'Appointment'" />
                        </div>
                    </div>
                    <div class="w-full my-2 flex">
                        <div class="w-6/12">
                            <auto-complete v-model="reqBody.progress" :label="'Progress'" :items="[ 'Appointment', 'Communication Request', 'Device Request', 'Medication Request', 'Nutrition Order' ]" />
                        </div>
                        <div class="w-6/12">
                    ...
                    <div class="w-full pt-5 my-4" v-if="outcomeReference?.id" style="border-top: 1px dashed #C2C7D6;">
                        <div class="w-full flex">
                            <div class="w-4/12 px-2 box" v-if="true">
                                <selected-author :selectedAuthor="outcomeReference" :title="'Outcome Reference'" />
                            </div>
                            <!-- <div class="w-4/12 ml-6" v-if="selectedContributor?.id">
                                <selected-contributor @remove="() => selectedContributor = {}" :selectedContributor="selectedContributor" />
                            </div> -->
                ...
                <div class="w-full">
                   <div class="w-full mb-3">
                        ...
                        <div class="w-full flex flex-wrap py-4">
                            ...
                            <div class="mx-3">
                                <label class="inline-flex items-center">
                                    <input v-model="reqBody.detail.instantiatesCanonical" type="radio" class="form-radio h-4 w-4" :value="'active'" >
                                    <span class="ml-2 text-sm">ActivityDefinition</span>
                                </label>
                            </div>
                            <div class="mx-3">
                    ...
                    <div class="w-full my-2 flex items-center">
                        <div class="w-6/12 -mt-3">
                            <cornie-input v-model="reqBody.detail.instantiatesUri" :label="'Instantiates Uri'"  />
                        </div>
                        <div class="w-6/12">
                            <auto-complete  v-model="reqBody.detail.code" :label="'Code'" :items="codes" />
                        </div>
                    </div>

                    <div class="w-full my-2 flex">
                        <div class="w-6/12">
                            <auto-complete v-model="reqBody.detail.reasonCode" :label="'Reason Code'" :items="reeasonReferences" />
                        </div>
                        <div class="w-6/12">
                    ...
                    <div class="w-full my-2 flex items-center">
                        ...
                        <div class="w-6/12">
                            <auto-complete v-model="reqBody.detail.status" :label="'Status'" :items="detailStatus" />
                        </div>
                    </div>

                    <div class="w-full my-2 flex items-center">
                        <div class="w-6/12 -mt-3">
                            <cornie-input v-model="reqBody.detail.statusReason" :label="'Status Reason'" />
                        </div>
                        <div class="w-6/12">
                            <div class="w-full mb-3">
                                ...
                                <div class="w-full flex flex-wrap py-4">
                                    <div class="mx-3">
                                        <label class="inline-flex items-center">
                                            <input v-model="reqBody.detail.doNotPerform" type="radio" class="form-radio h-4 w-4" :value="true" >
                                            <span class="ml-2 text-sm">True</span>
                                        </label>
                                    </div>
                                    <div class="mx-3">
                                        <label class="inline-flex items-center">
                                            <input v-model="reqBody.detail.doNotPerform" type="radio" class="form-radio h-4 w-4" :value="'False'" >
                ...
                <div class="w-full mb-3" style="height: 320px; overflow-y: scroll">
                    <div class="w-full mb-3">
                        ...
                        <div class="w-full flex flex-wrap py-4">
                            <div class="mx-3">
                                <label class="inline-flex items-center">
                                    <input v-model="reqBody.scheduleTiming.scheduleTiming" type="radio" class="form-radio h-4 w-4" :value="'planned'" >
                                    <span class="ml-2 text-sm">Day</span>
                                </label>
                            </div>
                            <div class="mx-3">
                                <label class="inline-flex items-center">
                                    <input v-model="reqBody.scheduleTiming.scheduleTiming" type="radio" class="form-radio h-4 w-4" :value="'active'" >
                    ...
                    <div class="w-full flex">
                        <div class="w-6/12">
                            <auto-complete v-model="reqBody.scheduleTiming.location" :label="'Location'" :items="[ 'Location 1', 'Location 2' ]" />
                        </div>
                        <div class="w-6/12">
                            <select-reference :label="'Performer'" @selected="performerSelected"
                                :options="[ 'Practitioner', 'Patient', 'Location', 'Device' ]"
            ...
            <template #form>
                <div class="w-full">
                    ...
                    <div class="w-full flex items-center mt-5 mb-3">
                        <div class="w-6/12">
                            <cornie-input v-model="reqBody.product.dailyAmount" :label="'Daily Amount'"  />
                        </div>
                        <div class="w-6/12 -mt-3">
                            <cornie-input v-model="reqBody.product.quantity" :label="'Quantity'" />
                        </div>
                    </div>
                    <div class="w-full flex items-center mt-5 mb-3">
                        <div class="w-6/12 -mt-3">
                            <cornie-input v-model="reqBody.product.description" :label="'Description'" />
                        </div>
                    </div>

                    <div class="w-full">
                        <text-area v-model="reqBody.product.note" :label="'Note'" style="width: 96%" />
                    </div>
                </div>
            </template>
        </collapse-section>

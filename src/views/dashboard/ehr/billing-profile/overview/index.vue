<template>
  <div class="flex justify-between space-x-7 p-5 w-full">
...
<script lang="ts">
  ...
  export default class BillingProfileOverview extends Vue {
    get patientId() {
    ...
    async fetchOveview() {
      const overview = cornieClient().get(
        `/api/v1/billing-profile/patient/${this.patientId}/overview`
      );

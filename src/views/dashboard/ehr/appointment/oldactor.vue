          ...
          <div v-if="actorType == 'Practitioner'">
            <div v-for="(input, index) in practitioners" :key="index">
              <div class="flex justify-between space-x-4 w-full mt-2 p-3">
                ...
                <div
                  class="w-full mb-5 cursor-pointer w-full text-xs float-right text-danger"
                >
...
export default {
  ...
  computed: {
    total() {
      return (
        this.updateHealthcare.length +
        this.updateDevices.length +
        this.updatePatients.length +
        this.updatePractitioners.length +
        this.updateLocation.length
      );
    },
    show: {
      get() {

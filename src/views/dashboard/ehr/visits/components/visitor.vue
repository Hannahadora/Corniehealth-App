            ...
            <div class="container-fluid pb-2">
                <div class="w-5/12 flex items-center">
                    <div class="w-1/12 rounded-full" style="max-height: 60px; max-width: 40px">
                        ...
                        <Avatar v-if="patient?.practitioner?.image" :src="patient?.practitioner?.image" />
                        <Avatar v-else :src="`https://via.placeholder.com/40x40`" class="w-full" />
                    </div>
                    <div class="w-11/12 ml-2">
                        <div class="w-full mb-1">
                            <p class="font-semibold text-sm mb-0"> {{ patient?.practitioner?.firstName }} {{ patient?.practitioner?.lastName }}</p>
                        </div>
                        <div class="w-full">
                            <span class="flex items-center text-sm">
                                <span class="uppercase text-success">Appointment Time {{ appointment?.duration?.split('-')[0]?.substring(0, 5)}}</span>
...
export default class CheckIn extends Vue {
    ...
    async created() {
        ...
        console.log(this.patients, "in details");

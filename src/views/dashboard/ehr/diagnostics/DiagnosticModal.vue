<template>
  ...
  />
  <reference v-model="showRefModal" @ref-value="refvalue" />
</template>
...
<script lang="ts">
  ...
  export default class MedicationModal extends Vue {
    ...
    get performerId() {
      return this.authPractitioner.organizationId;
    }
    ...
    get payload() {
      return {
        ...
        performerId: this.performerId || this.practiseId,
    ...
    async createRequest() {
      try {
        ...
        if (response.success) {
          window.notify({
            msg: "Diagnostic Request Created",
            status: "success",
          });
          this.done();
        } else {
        ...
        window.notify({
          msg: "Diagnostic Request Not Created",
          status: "error",
        });
      }
    }
    async updateRequest() {
      ...
      try {
        ...
        if (response.success) {
          ...
          window.notify({
            msg: "Diagnostic Request Updated",
            status: "success",
          });
          this.done();
        }
        ...
        window.notify({
          msg: "Diagnostic Request Not Updated",
          status: "error",
        });
      }
    }
    ...
    async created() {
      ...
      console.log("authh", this.authPractitioner);
      this.practiseId = this.authPractitioner.organizationId;
    }
  }
</script>

        ...
        <div class="w-full py-2" v-for="(item, index) in itemsList" :key="index" style="border-bottom: 1px solid #EBEFF2;">
          <div class="w-full flex">
            ...
            <div class="w-3/12 flex justify-end">
              <span class="tag flex flex items-center px-3 py-1" :class="{ 'new-item': item.status?.toLocaleLowerCase() !== 'completed', 'bg-success rounded-full': item.status?.toLocaleLowerCase() === 'completed' }" style="width: fit-content">
                <span>{{ item.status?.toLocaleLowerCase() !== 'completed' ? 'New' : 'Done' }}</span> 
                <span class="ml-2" v-if="item.status?.toLocaleLowerCase() !== 'completed'">
                  <info-icon />
                </span>
...
export default class HistoryCard extends Vue {
  ...
  get items() {
    ...
    return sortedList?.map(history => {
      const practitioner = this.practitioners?.find(practitioner => practitioner.id === history.practitionerId);
      return {
        code: history.conditionRelatedPerson?.code,
        date: formatFullDate(history.basicInfo?.date),
        practitioner: practitioner ? `${practitioner.firstName} ${practitioner.lastName}` : "Unknown",
        id: history.id,
  ...
  async created() {
    ...
    if (this.practitioners?.length <= 0) await this.fetchHistorys(this.$route.params.id.toString());

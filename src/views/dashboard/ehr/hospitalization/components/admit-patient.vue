    ...
    <div class="container-fluid p-3">
            ...
            <div class="w-full" style="overflow-y:scroll; height: 650px">
                <div class="w-full mb-2 flex items-center">
                    ...
                    <div class="w-6/12 mt-4">
                        <auto-complete :label="'Ward'" v-model="reqBody.ward" :items="[ 'Ward 1', 'Ward 2' ]" />
                    </div>
                </div>

                <div class="w-full my-2 flex items-center">
                    <div class="w-6/12">
                        <auto-complete :label="'Room'" v-model="reqBody.room" :items="[ '11', '12', '13', '14', '15' ]" />
                    </div>
                    <div class="w-6/12">
                        <auto-complete :label="'Bed'" v-model="reqBody.bed" :items="[ '20', '21', '22', '23', '24', '25'  ]" />
                    </div>
                </div>
                ...
                <div class="w-full my-2 flex items-center">
                    ...
                    <div class="w-6/12">
                        <auto-complete v-model="reqBody.reAdmission" :label="'Re-admission'" :items="[ 'Yes', 'No' ]" />
                    </div>
                </div>
                ...
                <div class="w-full my-2 flex items-center">
                    ...
                    <div class="w-6/12 -mt-5">
                        ...
                        <origin :value="testValue" @inpu="selectDestination" :label="'Destination'" />
                    </div>
                </div>
                ...
                <div class="w-full py-4 flex items-center" v-if="origin?.id || destination?.id" style="border-top: 1px dashed #C2C7D6;">
                    <div class="w-4/12" v-if="origin?.id">
                        ...
                        <div class="w-full styled-box p-4">
                            <p class="flex flex-col items-start justify-start">
                                <span style="color: #14171F" class="font-semibold">{{ origin?.name }}</span>
                                <span class="subtext" style="color: #667499">{{ origin?.address }}</span>
                            </p>
                        </div>
                    </div>
                    <div class="w-4/12 ml-5" v-if="destination?.id">
                        ...
                        <div class="w-full styled-box p-4">
                            <p class="flex flex-col items-start justify-start">
                                <span style="color: #14171F" class="font-semibold">{{ destination?.name }}</span>
                                <span class="subtext" style="color: #667499">{{ destination?.address }}</span>
                            </p>
                        </div>
                    </div>
        ...
        <div class="w-full">
            <div class=" flex justify-end">
                ...
                <CornieBtn v-if="hospitalisationId" :loading="loading" @click="onUpdate" class="bg-red-500 p-2 rounded-full px-8 mx-4">
                    <span class="text-white font-semibold">Update</span>
                </CornieBtn>

                <CornieBtn v-else :loading="loading" @click="admitPatient" class="bg-red-500 p-2 rounded-full px-8 mx-4">
...
export default class AdmitPatient extends Vue {
    ...
    updateReqBody() {
        if (this.hospitalisationId) {
            const hospitalisation = this.items?.find(hospitalisation => hospitalisation.id === this.hospitalisationId);

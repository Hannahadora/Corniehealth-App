<template>
  <div
    class="flex-col justify-center bg-white shadow-md p-3 mb-2 rounded w-full h-screen overflow-auto"
  >
      ...
      <div class="p-2">
        ...
        >
          ...
          <template #actions="{ item }">
            <table-action @click="viewProcedure(item)">
            ...
            <table-action @click="openUpdateStatusModal(item)">
...
import NewProcedure from "./components/new-procedure2.vue";
...
export default class ExistingState extends Vue {
  ...
  get selectedProcedure() {
    ...
    return (
      this.procedures.find(
        (procedure) => procedure.id === this.selectedProcedureId
      ) ?? {}
    );
  }
  ...
  async updateProcedureStatus() {
    try {
      ...
      const response = await this.updateProcedure(
        helperFunctions.formatReqBody(procedure)
      );

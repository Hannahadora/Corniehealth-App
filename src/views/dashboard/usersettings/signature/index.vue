<template>
    <div class="container-fluid h-screen topest" style="height: 100vh; overflow-y:scroll">
        <div class="w-full">
            ...
            <div class="w-full">
                <div class="container-fluid border-dashed drag-area py-6 flex items-center justify-center">
                    <div class="w-3/12 text-center">
                        <p class="my-3">Drag and Drop items here</p>
                        <p class="my-3">Or</p>
                        
                        <label for="x">
                            <span><p class="my-3"><a class="font-semibol cursor-pointer font-semibold">Browse Files</a></p></span>
                            <input type="file" name="x" id="x" @change="image.onChange" ref="filePicker" class="file-picker">
                        </label>
            ...
            <div class="w-full mt-8">
                ...
                <div class="container-fluid flex flex-col">
                    <label class="inline-flex items-center my-4" v-for="(document, index) in documents" :key="index">
                        <input type="checkbox" class="form-radio h-6 w-6" v-model="document.selected" name="schedule">
            ...
            <div class="w-full mt-8">
                <div class="w-full mt-6 mb-2 flex justify-end border-t-2 pt-8">
                    <Button :loading="loading" class="focus:outline-none" @click="updateSignature">
...
export default class UserSignature extends Vue {
    ...
    documents: any = [
        { code: 'Birth Certificate', display: 'Birth Certificate', selected: false },
        { code: 'Prescriptions', display: 'Prescriptions', selected: false },
        { code: 'Forms', display: 'Forms', selected: false },
        { code: 'Death Certificate', display: 'Death Certificate', selected: false },
    ]
    ...
    async updateSignature(e: any) {
        ...
        try {
            ...
            await this.postSignature({ 
                url: this.image.url,
                documents: this.documents.filter((i: any) => i.selected).map((i: any) => i.code),
            })
            this.loading = false;
            notify({

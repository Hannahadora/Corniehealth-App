<template>
    <div class="conatainer-fluid h-screen mt-6 bg-white p-4">
        <div class="w-full flex items-center">
            ...
            <CornieSelect v-model="data.type" :items="['Daily', 'Weekly', 'Monthly']" style="width: 150px" />
        </div>
        <div class="w-full px-2 flex">
            <div class="w-9/12">
                <div class="w-full flex" v-for="(item, index) in timelines" :key="index">
                    <div class="w-1/12 py-2 border-t-2 mx-1">
                        <span class="text-gray-500 text-xs font-semibold">{{ new Date(item.createdAt).toTimeString().split(" ")[0].substring(0, 5) }}</span>
                    </div>
                    <div class="w-11/12 py-2 flex border-t-2 items-center" style="background: #F3FCF8">
                        <div class="w-full flex items-center">
                        ...
                        <div class="w-3/12 text-right flex justify-end">
                            <span class="flex items-center">
                                <span class="uppercase text-xs font-semibold text-success">{{ item.action }}</span>
            ...
            <div class="w-3/12 ">
                ...
                <div class="w-full ml-3 border p-2">
                    <v-date-picker color="red" v-model="data.date" style="border: none"></v-date-picker>
                    <div class="w-12/12">
...
import AllActors from '../../schedules/components/actors.vue';
...
export default class TimelineTable extends Vue {
    ...
    get timelines() {
        ...
        const visit = this.visits.find((i: any) => i.id === this.$route.query.visit);
    ...
    get appointment() {
        ...
        const appmnt =  this.appointments.find((i: any) => i.id === visit.appointmentId);
        this.data.date = new Date(appmnt.period.start);
        this.data.time = new Date(appmnt.period.start).toTimeString().split(" ")[0].substring(0, 5);
        return appmnt;
    }

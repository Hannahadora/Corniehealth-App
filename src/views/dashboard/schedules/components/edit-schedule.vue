    ...
    <div class="w-full p-4 overflow-y-scroll h-scrren">
        <div class="container-fluid">
            <div class="w-full my-4">
                <cornie-input :label="'Schedule'" class="w-12/12" style="width: 100%" v-model="data.schedule" />
            </div>
                ...
                <div class="w-full flex">
                    <div class="mr-6" v-for="(state, index) in activeStates" :key="index">
                        <label class="inline-flex items-center">
                            <input type="radio" class="form-radio h-6 w-6"  v-model="state.code">
            ...
            <div class="w-full my-4">
                <CornieSelect :items="[1, 2, 3]" :label="'Service Category'" v-model="data.serviceCategory" style="width: 100%" />
            </div>

            <div class="w-full my-4">
                <CornieSelect :items="[1, 2, 3]" :label="'Service Type'" v-model="data.serviceType" style="width: 100%" />
            </div>

            <div class="w-full my-4">
                <CornieSelect :items="[1, 2, 3]" :label="'Specialty'" v-model="data.specialty" style="width: 100%" />
            </div>

            <div class="w-full my-4">
                <CornieSelect :items="[1, 2, 3]" :label="'Appointment'" v-model="data.appointmentType" style="width: 100%" />
            </div>

            <div class="w-full my-4">
                <cornie-input :label="'Descrription (optional)'" class="w-12/12" style="width: 100%" v-model="data.schedule" />
            </div>

            <div class="w-full my-4">
                <cornie-input :label="'Capacity (optional)'" class="w-12/12" style="width: 100%" v-model="data.schedule" />
            </div>
            ...
            <div class="w-full my-4">
                <label for="">Wait list on full capacity <span class="text-xs text-gray-500">(optional)</span></label>
                <div class="w-full flex">
            ...
            <div class="w-full my-4">
                <label for="">How often do you want this slot to occur?  <span class="text-xs text-gray-500">(optional) </span></label>
                <div class="w-full flex flex-wrap">
                    <div class="mr-6" v-for="(state, index) in slotOccurence" :key="index" style="max-width: fit-content">
                        <label class="inline-flex items-center">
                            <input type="radio" class="form-radio h-6 w-6"  v-model="state.code">
                            <span class="ml-2">{{ state.display }}</span>
                        </label>
            ...
            <div class="w-full my-4">
                ...
                <div class="w-full flex items-center">
                    ...
                    <span class="mx-3"><CornieSelect v-model="data.appointmentType" :items="['Weekly', 'Daily', 'Monthly' ]" style="width: 124px" /></span>
                </div>
            </div>
        </div>
        ...
        <div class="w-full my-4">
            ...
            <div class="w-full flex flex-wrap mt-1">
                <div class="mr-6" v-for="(state, index) in days" :key="index" style="max-width: fit-content">
                    <label class="inline-flex items-center">
                        <input type="checkbox" class="form-radio h-6 w-6"  v-model="state.code">
                        <span class="ml-2">{{ state.display }}</span>
                    </label>
                </div>
            </div>
                ...
                <div class="mr-6 my-2" v-for="(state, index) in ends" :key="index">
                    <label class="inline-flex items-center">
                        ...
                        <span class="ml-2 flex items-center">
                            {{ state.display }} 
                            <span class="mx-2" v-if="index === 1"><DatePicker v-model="data.date" /></span>
                             <span class="mx-2" v-if="index === 2"><CornieInput v-model="data.appointmentType" style="width: 45px" /> <span class="mx-2">Occurrence</span>
                            </span>
                        </span>
                    </label>
                </div>
        ...
        <div class="w-full my-4">
            <CornieSelect :items="[1, 2, 3]" :label="'Slot Status (optional)'" v-model="data.appointmentType" style="width: 100%" />
        </div>
        ...
        <div class="w-full my-4 flex items-center">
            ...
            <div class="w-4/12">
                <label for="">
                    ...
                    <input type="time" v-model="data.endTime" class="w-full border rounded-lg p-2" id="appt" required>
                </label>
            </div>
        </div>
        ...
        <div class="w-full my-4">
            <CornieInput :label="'Comment (optional)'" v-model="data.appointmentType" style="width: 100%" />
        </div>

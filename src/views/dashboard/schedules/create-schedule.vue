  ...
  <div class="w-full p-4 h-scrren" style="height: fit-content">
      ...
      <div class="w-full my-6">
        <div class="container-fluid">
          <div title="Shift Details" class="bg-white shadow-xl rounded-lg">
            <div
              class="w-full px-4 details-area"
              :class="{
                'details-area-full': showDetails,
                'details-area-fuller': showSelectArea,
              }"
            >
              <div
                class="w-full curved py-2"
                :class="{ 'border-b-2': showDetails }"
              >
              ...
              <div class="container-fluid my-4">
                <p
                  class="text-sm font-normal font-bold uppercase flex items-center cursor-pointer"
                  style="color: #fe4d3c"
                  @click="toggleSelectDisplay"
                >
                  <span class="mr-3">Select Actor(s) for this schedule </span
                  ><AddIcon />
                </p>
              </div>
              ...
              <div class="container-fluid my-3">
                      ...
                      <SearchBox :items="allDevices">
                        <template #item="data">
                          <p class="my-2 flex justify-between">
                            <span>{{ data.item.display }}</span>
                            <span
                              @click="deviceSelected(data.item)"
                              class="text-danger font-semibold text-lg cursor-pointer"
                              >Add</span
                            >
                          </p>
                ...
                <div class="w-full flex">
                  <div class="w-4/12 py-3">
                    <div class="w-11/12 mx-auto">
                      <p
                        class="text-sm font-normal font-bold uppercase flex items-center cursor-pointer py-2"
                        style="border-bottom: 1px dashed #c2c7d6"
                      >
                        <span class="mr-3">Practioners </span>
                      </p>
                      <div
                        class="w-full flex items-center"
                        v-for="(actor, index) in data.practitioners"
                        :key="index"
                      >
                        <div class="w-1/12">
                          <div class="w-full">
                        ...
                        <div class="w-1/12">
                          <span @click="removeActor(actor.code)"
                            ><DeleteIcon
                          /></span>
                        </div>
                      </div>
                  ...
                  <div class="w-4/12 py-3">
                    <div class="w-11/12 mx-auto">
                      <p
                        ...
                        style="border-bottom: 1px dashed #c2c7d6"
                      >
                        <span class="mr-3">Devices </span>
                      </p>
                      <div
                        class="w-full flex items-center"
                        v-for="(actor, index) in data.devices"
                        :key="index"
                      >
                        <div class="w-1/12">
                          <div class="w-full">
          ...
          <div title="Shift Timing" class="my-6 bg-white shadow-xl rounded-lg">
            <div
              class="w-full px-4 planning-area"
              :class="{ 'planning-area-full': showPlanning }"
            >
              <div
                class="w-full curved p-2"
                :class="{ 'border-b-2 py-3': showPlanning }"
              >
                <h2 class="flex justify-between w-full font-bold items-center">
                  <span>Planing Horizon</span>
                  <span @click="togglePlanningDisplay" class="cursor-pointer"
                    ><ChevronDown
                  /></span>
                </h2>
              </div>
              ...
              <div class="container-fluid py-3">
                ...
                <div class="w-full flex mt-2">
                  <div
                    class="mr-6"
                    v-for="(type, index) in schedulesTypes"
                    :key="index"
                  >
                  ...
                  <MultiSelect :label="'Apply To'">
                    ...
                    >
                      <div class="py-1" role="none">
                        ...
                        <a
                          class="text-gray-700 block px-4 py-2 text-sm flex items-center"
                          role="menuitem"
                          tabindex="-1"
                          id="menu-item-0"
              ...
              <div
                class="container-fluid pb-3 pt-3 flex justify-around dashed-bottom"
              >
                <div class="w-4/12">
                  <label for="">
                    <span class="uppercase font-bold text-xs"
                      >SLot Size(Mins/Hrs)</span
                    >
                    <input
                      type="number"
                      v-model="data.slotSize"
          ...
          <div title="Shift Breaks" class="mt-6 bg-white shadow-xl rounded-lg">
            ...
            >
              <div
                class="w-full curved py-2 my-2"
                :class="{ 'border-b-2 py-3': showBreaks }"
              >
                <h2 class="flex w-full justify-between items-center">
                  <span>
                    ...
                    <span class="primary-text mx-3 text-xs"
                      >(This section is optional and can be added if
                      desired)</span
                    >
                  </span>
                  <span @click="toggleBreaksDisplay" class="cursor-pointer"
                    ><ChevronDown
                  /></span>
                </h2>
              </div>
              ...
              <div class="w-full mb-4">
                <div class="container-fluid mb-8 flex justify-end items-center">
                  <Button>
                    <a
                      @click="addBreak"
                      style="background: #35ba83"
                      class="hover:bg-blue-700 cursor-pointer focus:outline-none text-white font-bold py-3 px-8 rounded-full"
                    >
                      Save
                    </a>
                  </Button>
              ...
              <div class="w-full mb-12">
                <div class="container-fluid mb-8 border-t-2 pt-2">
                  ...
                  <div
                    class="w-full flex"
                    v-for="(item, index) in data.breaks"
                    :key="index"
                  >
                    <div class="w-3/12">
                    ...
                    <div class="w-3/12">
                      <p class="flex justify-between">
                        <span class="text-sm text-gray-500">
                          {{ item.endTime }}</span
                        >
                        <span @click="removeBreak(index)"
                          ><DeleteIcon :fill="'red'" style="color: red"
                        /></span>
                      </p>
                    </div>
                  </div>
    ...
    <div class="w-full mb-12">
      <div class="container-fluid mb-8 flex justify-end items-center">
        ...
        <Button :loading="loading" v-if="!$route.params.scheduleId">
          <a
            @click="saveSchedule"
        ...
        <Button :loading="loading" v-else @click="saveSchedule">
          <a
            style="background: #e1e3ea"
            class="bg-red-500 hover:bg-blue-700 cursor-pointer focus:outline-none text-gray-500 font-bold py-3 px-8 rounded-full"
          >
            Update
          </a>
        </Button>
...
export default class Shift extends Vue {
  ...
  removeActor(id: any) {
    ...
    this.data.practitioners = this.data.practitioners.filter(
      (i: any) => i.code !== id
    );
  }
  ...
  async created() {
    ...
    if (!this.healthcares || this.healthcares.length === 0)
      await this.fetchHealthcares();
    if (!this.devices || this.devices.length === 0) await this.fetchDevices();
    if (this.$route.params.scheduleId) {
      const targetSchedule = this.schedules.find(
        (i: any) => i.id === this.$route.params.scheduleId
      );
      this.data = {
        ...targetSchedule,
  ...
  async saveSchedule() {
    const body = {
      ...
      startTime: this.data.startTime
        ? `${this.data.startTime.split(":")[0]}:${
            this.data.startTime.split(":")[1]
          }`
        : "",
      endTime: this.data.endTime
        ? `${this.data.endTime.split(":")[0]}:${
            this.data.endTime.split(":")[1]
          }`
        : "",
    };
    this.loading = true;

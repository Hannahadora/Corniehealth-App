            ...
            <div>
                ...
                <div class="w-full">
                    <div class="w-full flex">
                        <span>
                            <cornie-select :items="participantTypes" v-model="participantType" />
                        </span>
                        <span>
                            <div class="container">
                                <cornie-input >
                    ...
                    <div class="w-full pb-2">
                        <span>
                            <div class="py-1 px-1 border" role="none" style="background: #F6F8F9;border-radius: 4px;">
                                <div class="w-full flex relative items-center my-2" v-for="(actor, index) in participants" :key="index">
                                    <div class="w-11/12 flex" @click="selectPractitioner(actor)">
                                        ...
                                        <div class="w-11/12 ml-3">
                                            <p class="capitalize py-0 my-0 font-semibold text-sm">{{ actor.display }}</p>
                ...
                <div class="w-full flex justify-end mt-6">
                    <div class="w-full pb-8 flex flex justify-end">
                        <corniebtn class="bg-gray-300 p-2 rounded-full px-8 mx-4">
                            <span class="font-semibold text-gray-500 cursor-pointer" @click="() => $emit('closesidemodal')">Close</span>
                        </corniebtn>

                        <corniebtn class="bg-red-500 p-2 rounded-full px-8 mx-4">
                            <span class="text-white font-semibold cursor-pointer" @click="saveParticpants">Add</span>
                        </corniebtn>
                    </div>
                </div>
            </div>
...
export default class AllActors extends Vue {
    ...
    selectPractitioner(actor: any) {
        ...
        const index = this.selectedActors.findIndex((participant: any) => participant.code === actor.code)
        if (index < 0) {
            this.selectedActors.push({ id: actor.code, name: actor.display, jobDesignation: actor.job, type: actor.type })
        } else {
    ...
    saveParticpants() {
        this.$emit('participants', { type: this.participantType, participants: this.selectedActors });
    }

    get allActors() {
        return [
            { code: '1', display: 'Hello', type: 'Surgeon'}
        ]
    }

    get participants(): any[] {
        
        if (this.participantType === 'location') return this.locations.map(location => {
            return { code:location.id, display: location.name, location: 'location' }
        })
        if (this.participantType === 'device') return this.devices.map(device => {
            return { code:device.id, display: device.deviceName, type: 'device' }
        })
        return this.practitioners?.map(practitioner => {
            return { code:practitioner.id, display: practitioner.firstName + practitioner.lastName, job: practitioner.jobDesignation, type: 'practitioner' }
        })
    ...
    created() {
        this.participantType = this.participantTypes.find(type => type.code === 'practitioner')?.code ?? null
    }
}
</script>

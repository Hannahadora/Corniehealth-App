<template>
  <cornie-dialog v-model="show" right class="w-4/12 h-full">
      ...
      <cornie-card-text class="flex-grow scrollable">
        <v-form ref="form">
          ...
          <div class="w-full mt-5">
            <div
              class="flex w-full border-dashed border-b-2 border-gray-200 pb-5"
            >
              <div class="w-full">
            ...
            <div v-if="id">
              ...
              >
                ...
                <delete-icon
                  ...
                  @click="deletePatientItem(item.id, index)"
                />
              </div>
            </div>
            <div v-else>
          ...
          <div class="w-full mt-2 mb-2">
            ...
            >
              <div class="w-full">
                <span class="text-sm font-bold"
                  >Other Participants
                  <span class="text-xs text-gray-500">(Optional)</span></span
                >
              </div>
              <add-icon
                class="flex justify-end float-right cursor-pointer"
            ...
            <div v-if="id">
              <div
                class="w-full flex space-x-7 mt-4"
                v-for="(item, index) in Practitioners"
                :key="index"
              >
                <div class="w-full dflex space-x-4 mb-3">
                  <div class="w-10 h-10">
                ...
                <delete-icon
                  class="fill-current text-danger cursor-pointer"
                  @click="deletePractItem(item.id, index)"
                />
              </div>
              <div
                class="w-full flex space-x-7 mt-4"
            ...
            <template v-slot:default>
              <div class="w-full mt-5">
                <span class="text-primary font-semibold text-sm"
                  >Select where you’ll like appointment to be held</span
                >
    ...
    />
    <participant-modal
      v-model="showParticipant"
      :Practitioners="Practitioners"
      :id="id"
      :appoimtentId="appoimtentId"
...
<script lang="ts">
  ...
  import BillingAccountComponent from "@/components/billing-account-component.vue";
  ...
  export default class appointmentModal extends Vue {
    ...
    async createAppointment() {
      ...
      if (this.authCurrentLocation) {
        ...
        window.notify({
          msg: "Kindly switch default location",
          status: "error",
        });
      }
    }
    ...
    getAppointment() {
      if (this.Practitioners.length < 1) {
        const pt = this.practitioners.find(
          (i: any) => i.id === this.appoimtentId
        );
        this.practitionerId.push({
          practitionerId: this.appoimtentId,
          required: true,
        });

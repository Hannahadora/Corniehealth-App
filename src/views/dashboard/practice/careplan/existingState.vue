...
export default class ExistingState extends Vue {
  ...
  get items() {
    let careplans = this.careplans.map((careplan: any) => {
      ...
      return {
        ...
        action: careplan.id,
        address:
          careplan.practitioner.firstName +
          " " +
          careplan.practitioner.lastName,
        author: this.getPractitionerName(careplan.author),
        schedule:
          new Date(careplan?.scheduleTiming?.date)?.toLocaleDateString(
            "en-US"
          ) ||
          new Date(careplan?.scheduleTiming?.period?.start)?.toLocaleDateString(
            "en-US"
          ) +
            "-" +
            new Date(careplan?.scheduleTiming?.period?.end)?.toLocaleDateString(
              "en-US"
            ),
  ...
  get activePatientId() {
    const id = this.authPractitioner.organizationId as string;
    return id;
  }

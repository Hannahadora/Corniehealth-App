...
export default class statusDialog extends Vue {
  ...
  get updateDate() {
    return new Date((this.selectedItem as any).updatedAt).toLocaleDateString("en-NG");
  }

  get currentStatus() {
    return (this.selectedItem as any).status;
  }

  get updatedBy() {
    return this.findPractitioner(this.authPractitioner.id);
  }

  async updateStatus() {
    const id = this.selectedItem.id;
    const url = `/api/v1/catalogue-service/status/${id}`;
    const body = {

<template>
      ...
      <template #actions="{ item }">
        ...
        >
         ...
         <div class="flex items-center hover:bg-gray-100 p-3 cursor-pointer"  @click="showWaybillmodal(item.id)">
                ...
                <span class="ml-3 text-xs">Create Way bill</span>
           </div>
  ...
  <waybill-modal :requestId="waybillId" v-model="showWaybill" @waybillAdded="requestAdded"/>
</template>
<script lang="ts">
...
export default class IssuedExistingState extends Vue {
  ...
  async requestAdded() {
     if (this.authCurrentLocation) await this.fetchMaterialRequestsIncoming(this.authCurrentLocation);
  }

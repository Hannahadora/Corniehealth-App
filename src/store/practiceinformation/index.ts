...
export default {
    ...
    actions: {
        ...
        async getPracticeinformationById(ctx, id: string) {
            if (ctx.state.practiceInformations.length < 1)
                await ctx.dispatch("setPracticeInformations");
        ...
        async getPracticeHourById(ctx, id: string) {
            if (ctx.state.practiceHours.length < 1)
                await ctx.dispatch("setPracticeHours");

...
export default {
  ...
  mutations: {
    ...
    updateAssociations(state, associations: IAssociate[]) {
      const associationSet = new ObjectSet([...state.associations, ...associations], "id");
      state.associations = [...associationSet];
    },
    deleteAssociation(state, id: string) {
      const index = state.associations.findIndex((association) => association.id == id);
      if (index < 0) return;
  ...
  actions: {
    ...
    async getAssociationById(ctx, id: string) {
      if (ctx.state.associations.length < 1) await ctx.dispatch("fetchAssociations");

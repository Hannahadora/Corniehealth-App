...
export default {
  ...
  mutations: {
    ...
    updateCollectionAccounts(state, collectionAccounts: ICollection[]) {
      const collectionAccountSet = new ObjectSet(
        [...state.collectionAccounts, ...collectionAccounts],
        "id"
      );
    ...
    deleteCollectionAccount(state, id: string) {
      const index = state.collectionAccounts.findIndex(
        collectionAccount => collectionAccount.id == id
      );
      if (index < 0) return;
  ...
  actions: {
    ...
    async getCollectionAccountById(ctx, id: string) {
      if (ctx.state.collectionAccounts.length < 1)
        await ctx.dispatch("fetchCollectionAccounts");
      return ctx.state.collectionAccounts.find(
        collectionAccount => collectionAccount.id == id
      );
    },

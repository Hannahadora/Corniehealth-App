...
export default {
    ...
    mutations: {
        ...
        updateAppointmentTypes(state, appointmentTypes: IAppointmentTypes[]) {
            const appointmentTypeSet = new ObjectSet(
                [...state.appointmentTypes, ...appointmentTypes],
                "id"
            );
    ...
    actions: {
        ...
        async getAppointmentTypeById(ctx, id: string) {
            if (ctx.state.appointmentTypes.length < 1)
                await ctx.dispatch("fetchappointmentTypes");
            return ctx.state.appointmentTypes.find(
                appointmentType => appointmentType.id == id
            );
        },
